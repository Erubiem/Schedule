...
1.	[Google AdMob(Google)](#-1.-google-admob)
2.	MAX(Applovin)
3.	IronSource(Unity)
...
##	2.	Meta Audience (입찰만 받는다. 즉 광고 네트워크로만 가능하다는 뜻. 그러니 이건 메디에이션에서 제외)
...
# - 레전드 오브 슬라임의 광고 종류
 - 레전드 오브 슬라임의 광고 체계는 보상형 광고로 이루어졌지만, 자세히 보면 두 개로 나뉘어진다.

## 1. 보상형 동영상 광고
## 2. 보상형 전면 광고

이 두개의 치이로는 보상형 동영상 광고는 스킵이 안되고 보싱형 전면 광고는 일정 시간이 지난 후엔 스킵이 된다는 사실이다.
...
결국 레전드 오브 슬라임은 아이언소스의 루나로부터 솔루션(분석)을 받아 수익이 올라갔다고 하는데  
